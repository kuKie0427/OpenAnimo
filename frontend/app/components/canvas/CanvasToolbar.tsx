...
export const CanvasToolbar = track(function CanvasToolbar({
  ...
  const [exporting, setExporting] = useState<"pdf" | "webtoon" | "vertical" | null>(null);
  ...
  const handleExport = useCallback(
    async (format: "pdf" | "webtoon" | "vertical") => {
      if (!projectId) return;
      setExportMenuOpen(false);
      setExporting(format);
        ...
        const resp =
          format === "pdf"
            ...
            : format === "webtoon"
              ...
              : await exportApi.triggerVertical(projectId);
  ...
  return (
    <>
    <div
      className={`absolute bottom-4 left-1/2 -translate-x-1/2 z-50 flex items-center gap-1 p-1.5 rounded-lg bg-base-100 border border-base-content/10 shadow-gate-glow text-base-content ${className || ""}`}
    >
      {/* 选择工具 */}
                ...
                >
                  ...
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      d="M3 4v16h18V4H3zm2 2h14v12H5V6zm3 3h8v2H8V9zm0 4h8v2H8v-2z"
                    />
                  </svg>
                  导出 9:16 竖屏视频
                </button>
