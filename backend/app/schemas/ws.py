...
class RunAwaitingConfirmEventData(BaseModel):
    ...
    story_outline: StoryOutlineRead | None = None
    visual_bible: str | None = None
    entity_type: str | None = Field(default=None, description="被审批的实体类型: character | shot")
    entity_ids: list[int] | None = Field(default=None, description="被审批的实体 ID 列表")
    entity_summaries: list[dict] | None = Field(
        default=None,
        description="逐实体摘要列表（含 entity_id/name/description/image_url/approval_state）",
    )
...
class CritiqueEntityResult(BaseModel):
    entity_id: int
    entity_name: str
    score: float
    dimensions: dict[str, int] = Field(default_factory=dict)
...
class CritiqueReviewEventData(BaseModel):
    entity_type: str  # "character" | "shot"
    total: int
    results: list[CritiqueEntityResult]
...
class ShotSyncProgressEventData(BaseModel):
    paragraph_index: int
    status: str


class StageCompletenessWarningEventData(BaseModel):
    """Emitted after a production stage when artifacts are incomplete."""
    stage: str = ""
    missing_count: int = 0
    total: int = 0
    detail: str = ""
...
# Map of event types to their corresponding data models.
# Events with empty data (connected, pong, echo) map to None.
EVENT_DATA_MODELS: dict[str, type[BaseModel] | None] = {
    "connected": None,
    "pong": None,
    "echo": None,
    ...
    "critique_review": CritiqueReviewEventData,
    ...
    "shot_sync_progress": ShotSyncProgressEventData,
    "stage_completeness_warning": StageCompletenessWarningEventData,
}
