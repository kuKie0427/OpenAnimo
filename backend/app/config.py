...
class Settings(BaseSettings):
    ...
    app_name: str = "openanimo-backend"
    ...
    database_url: str = Field(
        default="postgresql+asyncpg://openanimo:openanimo_dev@localhost:5432/openanimo"
    )
    db_echo: bool = False
    ...
    image_size: str = Field(
        default="1024x1024",
        description="图像生成默认尺寸（DALL-E 用像素，Seedream 支持 2K/3K/4K 或 2560x1440+）",
    )
    image_output_format: str | None = Field(
        default=None,
        description="图像输出格式（png/jpeg），None 表示不传此参数",
    )
    ...
    critique_score_threshold: float = Field(
        default=6.0,
        description="加权总分阈值，低于此分数触发重新生成",
    )
    critique_consistency_threshold: int = Field(
        default=4,
        ge=0, le=10,
        description="一致性维度最低分（角色面部/服饰连贯性），低于此分触发重新生成（一票否决）",
    )
    critique_quality_threshold: int = Field(
        default=4,
        ge=0, le=10,
        description="质量维度最低分（图像清晰度/伪影），低于此分触发重新生成",
    )
    critique_composition_threshold: int = Field(
        default=4,
        ge=0, le=10,
        description="构图维度最低分（布局/透视/灯光），低于此分触发重新生成",
    )
    ...
    # 多角度角色资产生成
    multi_angle_enabled: bool = Field(
        default=False,
        description="是否启用多角度角色资产生成（4 视角 × 4 情绪）。默认 OFF 不影响现有用户。",
    )
    ...
    stub_services: bool = Field(
        default=False,
        description="一键启用所有 Fake 服务（本地开发/测试，不调用外部 API）",
    )
    ...
    pricing_json: str = Field(
        default="",
        description="JSON string to override DEFAULT_PRICING (e.g., '{\"gpt-4o\": "
    ...
    def _apply_stub_services(self):
        if self.stub_services:
            self.text_provider = "fake"
            self.image_provider = "fake"
            self.video_provider = "fake"
        return self
...
def get_settings() -> Settings:
    ...
    return Settings(_env_file=str(env_file), _env_file_encoding="utf-8")  # pyright: ignore[reportCallIssue]
