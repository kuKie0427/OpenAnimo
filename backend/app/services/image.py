...
class ImageService:
    ...
    async def generate(
        ...
        **kwargs: Any,
        ...
        if "/chat/completions" in self.settings.image_endpoint:
            ...
            if style:
                payload["style"] = style
        fmt = self.settings.image_output_format
        if fmt and "output_format" not in payload:
    ...
    async def generate_url(
        ...
        **kwargs: Any,
        ...
        effective_size = size or self.settings.image_size
        ...
        if image_bytes is not None and self.settings.use_i2i():
            try:
                ...
                if "/chat/completions" in self.settings.image_endpoint:
                    ...
                    fmt = self.settings.image_output_format
                    if fmt:
                        payload.setdefault("output_format", fmt)
