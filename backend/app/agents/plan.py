...
class PlanAgent(BaseAgent):
    ...
    async def _get_existing_state(self, ctx: AgentContext) -> dict[str, Any]:
        char_res = await ctx.session.execute(
            select(Character).where(Character.project_id == ctx.project_id)
        )
        characters = [
        ...
        shot_res = await ctx.session.execute(
            select(Shot).where(Shot.project_id == ctx.project_id).order_by(Shot.order)
        )
        shots = [
            {
                "id": s.id,
    ...
    async def _apply_incremental_changes(
        ...
        deleted_shot_ids = []
        shot_res = await ctx.session.execute(select(Shot).where(Shot.project_id == ctx.project_id))
        ...
        if isinstance(raw_characters, list):
            for item in raw_characters:
                ...
                if char_id is None:
                    ...
                    if existing_char and existing_char.project_id == ctx.project_id:
        ...
        if isinstance(raw_shots, list):
            for idx, shot_data in enumerate(raw_shots):
                ...
                if shot_id is None:
                    ...
                    if existing_shot and existing_shot.project_id == ctx.project_id:
    ...
    async def _call_plan_llm(
        ...
        *,
        ...
        if outline_context is not None:
            ...
            target_shot_count = getattr(ctx.project, "target_shot_count", None)
            if isinstance(target_shot_count, int) and target_shot_count > 0:
                from app.services.beat_analyzer import analyze_beats, format_beat_schedule

                acts = outline_context.get("story_outline", {}).get("acts", [])
                beat_assignments = analyze_beats(target_shot_count, acts)
                payload["beat_schedule"] = format_beat_schedule(beat_assignments)
        ...
        try:
            ...
            logger.exception(
                "Failed to parse PlanAgent LLM response (task=%s, len=%d): %s",
                task, len(resp.text), resp.text[:2000],
            )
            raise
        ...
        if updated_fields:
            await ctx.ws.send_event(
                ctx.project_id,
                {
                    ...
                    "data": {"project": {"id": ctx.project_id, **updated_fields}},
                },
            )
    ...
    async def run_characters(self, ctx: AgentContext) -> None:
        async with self.trace_step(ctx, stage="plan_characters", method="run_characters", model_name=ctx.settings.text_model) as trace:
            is_incremental = ctx.rerun_mode == "incremental"
            if is_incremental:
            ...
            if getattr(ctx.project, "universe_id", None) and not is_incremental:
                try:
                    from app.services.universe_service import UniverseService
                    svc = UniverseService(ctx.session)
                    imported = await svc.auto_import_shared_characters(ctx.project_id)
                    if imported:
            ...
            data = await self._call_plan_llm(ctx, task="characters", trace=trace)
            ...
            raw_characters = data.get("characters") or []
            lines: list[str] = []
            await ctx.session.execute(delete(Character).where(Character.project_id == ctx.project_id))
            await ctx.session.flush()
    ...
    async def run_shots(self, ctx: AgentContext) -> None:
        async with self.trace_step(ctx, stage="plan_shots", method="run_shots", model_name=ctx.settings.text_model) as trace:
            char_res = await ctx.session.execute(
                select(Character).where(Character.project_id == ctx.project_id)
            )
            ...
            if (
                ...
                data = await self._call_plan_llm(ctx, task="shots", characters=characters, trace=trace)
            ...
            if is_incremental:
                ...
                shot_res = await ctx.session.execute(
                    select(Shot).where(Shot.project_id == ctx.project_id).order_by(Shot.order.asc())
                )
            ...
            await ctx.session.execute(delete(Shot).where(Shot.project_id == ctx.project_id))
            await ctx.session.flush()
