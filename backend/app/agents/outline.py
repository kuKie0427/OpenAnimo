...
class OutlineAgent(BaseAgent):
    ...
    async def run_outline(self, ctx: AgentContext) -> None:
        async with self.trace_step(ctx, stage="plan_outline", method="run_outline", model_name=ctx.settings.text_model) as trace:
            ...
            try:
                ...
                logger.exception(
                    "Failed to parse OutlineAgent LLM response (len=%d): %s",
                    len(resp.text), resp.text[:2000],
                )
                raise
            ...
            raw_update = data.get("project_update")
            project_update: dict[str, Any] = raw_update if isinstance(raw_update, dict) else {}

            ctx.project.story_outline = dict(outline)
            ctx.project.visual_bible = (visual_bible or "").strip() or None
