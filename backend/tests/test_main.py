...
async def test_ws_projects_handles_ping_and_echo(monkeypatch):
    ...
    monkeypatch.setattr("app.orchestration.redis.get_awaiting_payload", lambda run_id: None)
    monkeypatch.setattr(
        main_module,
        "get_settings",
...
async def test_ws_projects_replays_awaiting_payload(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "get_settings",
        lambda: SimpleNamespace(
            app_name="openanimo-backend", cors_origins=[], api_v1_prefix="/api/v1", environment="development"
        ),
    )
    monkeypatch.setattr(main_module, "init_db", lambda: None)
    monkeypatch.setattr("app.db.session.async_session_maker", FakeAsyncSessionMaker())
    ...
    monkeypatch.setattr("app.orchestration.redis.get_awaiting_payload", fake_get_awaiting_payload)
...
async def test_ws_projects_confirm_invalid_run_sends_error(monkeypatch):
    ...
    monkeypatch.setattr("app.orchestration.redis.trigger_confirm_redis", fake_trigger)
